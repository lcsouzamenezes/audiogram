...
Audiogram.prototype.getWaveform = function (cb) {
  ...
  probe(this.audioPath, function (err, data) {
    ...
    if (
      self.settings.theme.maxDuration &&
      self.settings.theme.maxDuration < data.duration
    ) {
      return cb(
        "Exceeds max duration of " + self.settings.theme.maxDuration + "s"
      );
    }
    ...
    self.set(
      "numFrames",
      (self.numFrames = Math.floor(
        data.duration * self.settings.theme.framesPerSecond
      ))
    );
