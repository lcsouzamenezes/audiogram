...
module.exports = function (themes) {
  if (!themes || !_.keys(_.omit(themes, "default")).length) {
    return console.warn(
      "No themes found in settings/themes.json. See https://github.com/nypublicradio/audiogram/blob/master/THEMES.md for details."
    );
  }
...
function validateTheme(name, options) {
  ...
  ["width", "height", "framesPerSecond", "samplesPerFrame"].forEach(function (
    key
  ) {
  ...
  if (typeof options.backgroundImage === "string") {
    ...
    if (!path.isAbsolute(options.backgroundImage)) {
      fullBackgroundImagePath = path.join(
        __dirname,
        "..",
        "..",
